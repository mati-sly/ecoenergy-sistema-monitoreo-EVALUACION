# usuarios/views.py - Versión simplificada sin validaciones
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        login(request, user)
        return redirect('dashboard')  # Cambiado para que coincida con tu urls.py
...
# Vistas simplificadas para reset de password (solo render template)
def password_reset_view(request):
    """Vista de reset de password - solo muestra template"""
    if request.method == 'POST':
...
def verify_code_view(request):
    """Vista de verificación - solo muestra template"""
    if request.method == 'POST':
...
def reset_password_confirm(request, token=None):
    """Vista de confirmación - solo muestra template"""
    if request.method == 'POST':
